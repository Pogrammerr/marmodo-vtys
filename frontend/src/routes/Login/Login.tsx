...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (e.response.data.message === 'Wrong Password!' || e.response.data.message === 'Girilen Email ile kayıtlı kullanıcı bulunamadı.')
    ...
    <Layout style={{ justifyContent: 'center' }}>
      <Card size='md'>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          {error && <Text color='fail' style={{ marginTop: '-1rem' }}>{error}</Text>}
        </Form>
